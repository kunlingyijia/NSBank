...
- (void)hgdqGotoNative:(PGMethod *)commands{
    if (commands) {
        ...
        NSLog(@"cbid = %@",cbid);
//        // 跳转到原生界面
//        NewViewController *hgdqVC = [[NewViewController alloc] init];
//        hgdqVC.dic = (NSDictionary *)revInfo;
//        hgdqVC.NewViewControllerBlock =^(NSString * str){
//            //原生界面向H5界面传值
//            NSDictionary *cDice = @{@"name":str};
...
//        };
        ...
        UINavigationController * Nac = [[UINavigationController alloc]initWithRootViewController:customDetectVC];
        customDetectVC.CustomDetectViewControllerBlock = ^(NSString* path ,NSString * name, NSString * code){
            //原生界面向H5界面传值
                        NSDictionary *cDice = @{@"path":path,@"name": name,@"code":code};
                        PDRPluginResult *result = [PDRPluginResult resultWithStatus:PDRCommandStatusOK messageAsDictionary:cDice];
                        [self toCallback:cbid withReslut:[result toJSONString]];
        };
        [rootVC presentViewController:Nac animated:YES completion:nil];
        
        
        
        
        
        [[LJContactManager sharedInstance] selectContactAtController:[self getCurrentUIVC] complection:^(NSString *name, NSString *phone) {
...
-(void)addressBookNative:(PGMethod *)commands{
    if (commands) {
        ...
        [[LJContactManager sharedInstance] selectContactAtController:[self getCurrentUIVC] complection:^(NSString *name, NSString *phone) {
            //原生界面向H5界面传值
            NSDictionary *cDice = @{@"name": name,@"phone":phone};
            PDRPluginResult *result = [PDRPluginResult resultWithStatus:PDRCommandStatusOK messageAsDictionary:cDice];
            [self toCallback:cbid withReslut:[result toJSONString]];
        }];
